...
C g  u  u-bar  d  d-bar  s  s-bar  c  c-bar  b  b-bar
      ...
      real, dimension(0:10) :: DH
      DIMENSION FF(0:5),FF_pion(0:5),
     &  FF_kaon(0:5),FF_proton(0:5)
      ...
      call akk_pion(z,Q2,FF_pion)
      call akk_kaon(z,Q2,FF_kaon)
      call akk_proton(z,Q2,FF_proton)
      ...
      call akk_pionCSA(z,Q2,FF_pion)
      call akk_kaonCSA(z,Q2,FF_kaon)
      call akk_protonCSA(z,Q2,FF_proton)
      DH(0)=FF_pion(0)+FF_kaon(0)+FF_proton(0)
      DH(1)=FF_pion(1)+FF_kaon(1)+FF_proton(1)
      DH(2)=DH(1)
      DH(3)=FF_pion(2)+FF_kaon(2)+FF_proton(2)
...
c     The subroutines akk_ff(x,Q2,FF), for ff=pion, kaon and proton
...
c     Likewise, the subroutines akk_ffCSA(x,Q2,FF), for ff=pion, kaon
...
c     The subroutine akk_lambdaQCD(lambdaQCD) returns the QCD
...
c     subroutines akk_ff, akk_lambdaQCD and akk_unit described above
...
c     Returns all 6 pion FFs from AKK fit evaluated at (x,Q2)
     ...
     .     dpion_a(nQ2,nx,0:5)
      ...
      if(precalc_done.ne.2375)then         
         call akk_unit
         ! modified
         open(unit=grid_read,file='./data-grids/grid_pion',
     .        status='old')
         do i=1,nQ2
            do j=1,nx
...
c     Returns all 6 kaon FFs from AKK fit evaluated at (x,Q2)
     ...
     .     dkaon_a(nQ2,nx,0:5)
      ...
      if(precalc_done.ne.2375)then
         call akk_unit
         ! modified
         open(unit=grid_read,file='./data-grids/grid_kaon',
     .        status='old')
         do i=1,nQ2
            do j=1,nx
               read(grid_read,'(6e14.6)')
...
c     Returns all 6 proton FFs from AKK fit evaluated at (x,Q2)
      ...
      subroutine akk_proton(x,Q2,FFproton)
     ...
     .     cproton_a,dproton_a
      ...
      if(precalc_done.ne.2375)then
         call akk_unit
         ! modified
         open(unit=grid_read,file='./data-grids/grid_proton'
     .        ,status='old')
         do i=1,nQ2
            do j=1,nx
               read(grid_read,'(6e14.6)')
     ...
     .        cproton_a,dproton_a)
      ...
      call getFFs(x,Q2,FFproton,FFproton_a,
...
c     Returns all 6 pionCSA FFs from AKK fit evaluated at (x,Q2)
     ...
     .     dpionCSA_a(nQ2,nx,0:5)
      ...
      if(precalc_done.ne.2375)then         
         call akk_unit
         ! modified
         open(unit=grid_read,file='./data-grids/grid_pionCSA',
     .        status='old')
         do i=1,nQ2
            do j=1,nx
               read(grid_read,'(6e14.6)')
...
c     Returns all 6 kaonCSA FFs from AKK fit evaluated at (x,Q2)
     ...
     .     dkaonCSA_a(nQ2,nx,0:5)
      ...
      if(precalc_done.ne.2375)then
         call akk_unit
         ! modified
         open(unit=grid_read,file='./data-grids/grid_kaonCSA',
     .        status='old')
         do i=1,nQ2
            do j=1,nx
               read(grid_read,'(6e14.6)')
...
c     Returns all 6 protonCSA FFs from AKK fit evaluated at (x,Q2)
      ...
      subroutine akk_protonCSA(x,Q2,FFprotonCSA)
     ...
     .     cprotonCSA_a,dprotonCSA_a
      ...
      if(precalc_done.ne.2375)then
         call akk_unit
         ! modified
         open(unit=grid_read,file='./data-grids/grid_protonCSA'
     .        ,status='old')
         do i=1,nQ2
            do j=1,nx
               read(grid_read,'(6e14.6)')
     ...
     .              FFprotonCSA_a(i,j,5)
         ...
         enddo
         close(grid_read)
         call spline_fit(FFprotonCSA_a,bprotonCSA_a,
     .        cprotonCSA_a,dprotonCSA_a)
         precalc_done=2375
      endif
      call getFFs(x,Q2,FFprotonCSA,FFprotonCSA_a,
...
c     Returns all 6 K0S FFs from AKK fit evaluated at (x,Q2)
     ...
     .     dK0S_a(nQ2,nx,0:5)
      ...
      if(precalc_done.ne.2375)then
         call akk_unit
         ! modified
         open(unit=grid_read,file='./data-grids/grid_K0S',
     .        status='old')
         do i=1,nQ2
            do j=1,nx
               read(grid_read,'(6e14.6)')
     ...
     .        cK0S_a,dK0S_a)
      ...
      call getFFs(x,Q2,FFK0S,FFK0S_a,bK0S_a,
     .     cK0S_a,dK0S_a)
...
c     Returns all 6 Lambda FFs from AKK fit evaluated at (x,Q2)
     ...
     .     cLambda_a,dLambda_a
      ...
      if(precalc_done.ne.2375)then
         call akk_unit
         ! modified
         open(unit=grid_read,file='./data-grids/grid_Lambda',
     .        status='old')
         do i=1,nQ2
            do j=1,nx
               read(grid_read,'(6e14.6)')
...
c------------------------------------------------------------------------
      ...
      subroutine akk_lambdaQCD(lambdaQCD)
      ...
      call akk_unit
      ! modified
      open(unit=grid_read,file='./data-grids/lambdaQCD',
     .     status='old')
...
c------------------------------------------------------------------------
      ...
      ! modified
      open(unit=grid_read,file='./data-grids/Q2_grid',
     .     status='old')
      do i=1,nQ2-4,5
     ...
     .        Q2_a(i+3),Q2_a(i+4)
      ...
      ! modified
      open(unit=grid_read,file='./data-grids/x_grid',
     .     status='old')
      do i=1,nx-4,5
