...
!BL
     ...
     >             minexp,maxexp,eps,epsneg,xmin,xmax)
            USE nrtype
            INTEGER(I4B), INTENT(OUT) :: ibeta,iexp,irnd,it,machep,
     >             maxexp,minexp,negep,ngrd
      ...
      END INTERFACE
      INTERFACE
            SUBROUTINE mrqmin(x,y,sig,a,maska,covar,alpha,chisq,funcs,
     >            alamda)
            USE nrtype
            REAL(SP), DIMENSION(:), INTENT(IN) :: x,y,sig
            REAL(SP), DIMENSION(:), INTENT(INOUT) :: a
...
!BL
      ...
      INTERFACE
            ...
            REAL(DP), DIMENSION((size(uf,1)+1)/2,
     >            (size(uf,1)+1)/2) :: rstrct
            END FUNCTION rstrct
      END INTERFACE
      INTERFACE
...
!      and put it on the next one after that.
      ...
      END INTERFACE
      INTERFACE
            SUBROUTINE vegas(region,func,init,ncall,itmx,nprn,tgral,
     >            sd,chi2a)
            USE nrtype
