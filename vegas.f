      ...
      INTEGER(I4B), PARAMETER :: MXDIM=15,NDMX=50
      INTEGER(I4B), SAVE :: i,it,j,k,mds,nd,ndim,ndo,ng,npg
      INTEGER(I4B), DIMENSION(MXDIM), SAVE :: ia,kg
      REAL(SP), SAVE :: calls,dv2g,dxg,f,f2,f2b,fb,rc,ti,tsi,
     >  wgt,xjac,xn,xnd,xo,harvest
     ...
     >             ALPH,mds,nd,(j,region(j),j,region(j+ndim),j=1,ndim)
      ...
      do it=1,itmx
            ...
            iterate: do
                  ...
                  if(f2b*npg.lt.0d0) call exit(31) !!!
                  f2b=sqrt(f2b*npg)
                  f2b=(f2b-fb)*(f2b+fb)
                  if (f2b <= 0.0) f2b=TINY
                  ti=ti+fb
            ...
            if(1.0_sp/swgt.le.0_sp) call exit(32) !!!
            sd=sqrt(1.0_sp/swgt)
            if(tsi.le.0_sp) call exit(33) !!!
            tsi=sqrt(tsi)
            if (nprn >= 0) then
...
200      format(/' input parameters for vegas:  ndim=',
     >      i3,'  ncall=',f8.0
     >     /28x,'  it=',i5,'  itmx=',i5
     >     /28x,'  nprn=',i3,'  alph=',f5.2/28x,'  mds=',
     >      i3,'   nd=',i4
     >     /(30x,'xl(',i2,')= ',g11.4,' xu(',i2,')= ',g11.4))
201      format(/' iteration no.',
     >      I3,': ','integral =',g14.7,' +/- ',g9.2,
